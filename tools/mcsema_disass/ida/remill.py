...
class MainWindow(PluginForm):
    def OnCreate(self, form):
        ...
        idaapi.set_dock_pos("Remill", "IDA View-A", idaapi.DP_RIGHT)
    ...
    def initializeWidgets(self, layout):
        ...
        about_text.setText("<p>Remill<br/>Machine code to LLVM binary translator<br/><br/><a href=\"https://trailofbits.com\" style=\"color: {};\">https://trailofbits.com</a><br/><a href=\"https://github.com/trailofbits/remill\" style=\"color: {};\">https://github.com/trailofbits/remill</a></p>".format(link_color, link_color))
    ...
    def onExportButtonClick(self):
        ...
        entry_point = self._settings_page.entryPoint()
        ...
        verbose = self._settings_page._verbose.checkState() == QtCore.Qt.Checked

        base_output_path = idc.GetIdbPath()[:-4]
        output_file_path = base_output_path + ".ir"
        log_file_path = base_output_path + ".log"
        symbols_file_path = base_output_path + ".symbols"

        PrintMessage("New session: " + str(datetime.datetime.now()))
        PrintMessage("Architecture: " + architecture)
        PrintMessage("Operating_system: " + operating_system)
        PrintMessage("PIE mode: " + str(pie_mode))
        PrintMessage("Entry point: " + entry_point)

        if not get_cfg.exportCfg(log_file_path, architecture, pie_mode, operating_system, standard_definition_file_list, symbols_file_path, entry_point, output_file_path, False):
            PrintMessage("Export failed!")
            if verbose:
                print("===")
            ...
            PrintMessage("CFG exported to the following file: " + output_file_path)
...
class SettingsPage(QtWidgets.QWidget):
    ...
    def initializeWidgets(self, layout):
        ...
        self._architecture.addItem("aarch64")
        ...
        # flags
        self._pie_mode = QtWidgets.QCheckBox("Position Indipendent Executable (PIE)")
        settings_layout.addRow("Flags", self._pie_mode)

        self._verbose = QtWidgets.QCheckBox("Verbose output")
        settings_layout.addRow("", self._verbose)
        
        # entry point selector
        self._selected_entry_point = QtWidgets.QLabel("")
        settings_layout.addRow("Entry point", QtWidgets.QWidget())

        self._ida_function_list = QtWidgets.QListWidget()
        settings_layout.addRow(self._ida_function_list)
    ...
    def refresh(self):
        ...
        elif processor_name == 'ARMB':
            PrintMessage("Big endianness is not yet supported on ARM")

        elif processor_name == 'ARM':
            if not idaapi.get_inf_structure().is_64bit():
                PrintMessage("ARM is only supported for 64-bit executables")
                return

            self._architecture.setCurrentIndex(2)
        ...
        # update the function list for the entry point selector
        current_entry_point = self._ida_function_list.currentItem()
        if current_entry_point is None:
            current_entry_point = ""
        else:
            current_entry_point = current_entry_point.text()

        self._ida_function_list.clear()
        for function_name in function_name_list:
            self._ida_function_list.addItem(function_name)

        # update the selected entry point
        self.setEntryPoint(current_entry_point)

    def setEntryPoint(self, function_name):
        if function_name:
            search_result = self._ida_function_list.findItems(function_name, QtCore.Qt.MatchExactly)
            if len(search_result) != 0:
                self._ida_function_list.setCurrentItem(search_result[0])
                return True

            return False

        else:
            if self.setEntryPoint("main"):
                PrintMessage("Entry point automatically set to 'main'")
                return True

            elif self.setEntryPoint("start"):
                PrintMessage("Entry point automatically set to 'start'")
                return True

            PrintMessage("No suitable entry point found. Please select it manually!")
            return False
    ...
    def entryPoint(self):
        ...
        return current_entry_point.text()

    def architecture(self):
        return self._architecture.currentText()

    def operatingSystem(self):
        return self._operating_system.currentText()
...
def PrintMessage(message):
    print 'Remill - ' + message

main_window = MainWindow()
main_window.Show("Remill")